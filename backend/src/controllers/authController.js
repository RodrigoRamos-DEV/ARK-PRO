...
exports.login = async (req, res) => {
  ...
  try {
    // QUERY ATUALIZADA: Agora busca o nome da empresa junto com os dados do utilizador
    const userResult = await db.query(
        'SELECT u.*, c.company_name FROM users u LEFT JOIN clients c ON u.client_id = c.id WHERE u.email = $1',
        [email]
    );
    ...
    // PAYLOAD ATUALIZADO: Inclui o company_name
    const payload = {
      user: {
        id: user.id,
        clientId: user.client_id,
        role: user.role,
        companyName: user.company_name // Novo campo
      }
    };
...
exports.registerClient = async (req, res) => {
    ...
    try {
        ...
        const tokenResult = await db.query(
            'SELECT * FROM registration_tokens WHERE token_hash = $1 AND is_used = FALSE AND expires_at > NOW()',
            [tokenHash]
        );
        if (tokenResult.rows.length === 0) {
            return res.status(400).json({ msg: "Token de registo inválido, expirado ou já utilizado." });
        }
        ...
        if (existingUser.rowCount > 0) {
            return res.status(400).json({ msg: "Já existe um utilizador registado para este cliente." });
        }
        const salt = await bcrypt.genSalt(10);
        const passwordHash = await bcrypt.hash(password, salt);
        await db.query(
            'INSERT INTO users (client_id, email, password_hash, role) VALUES ($1, $2, $3, $4)',
            [client_id, email, passwordHash, 'funcionario']
        );
...
exports.forgotPassword = async (req, res) => {
    ...
    try {
        ...
        await transporter.sendMail({
            from: `"Sistemas ARK" <${process.env.EMAIL_USER}>`,
            to: user.email,
            subject: 'Redefinição de Senha - Sistemas ARK',
            html: message,
        });
